...
bool checkValidString(QString pass) { // check if username or pass string is invalid or valid (1 is valid)
	...
	for (int i = 0; i < a.length(); ++i) {
		if (a[i] < 45 || (a[i] > 57 && a[i] < 65) || (a[i] > 90 && a[i] < 94) || a[i] > 122 || a[i] == 47 || (a[i] > 96 && a[i] < 97)) {
			return 0;
			break;
		}
	}
...
bool checkLogin(QString user, QString pass) {
	...
	CONNECTDB
		con->setSchema(DBACC);
	stmt = con->createStatement();
	res = stmt->executeQuery("select * from account where available = '1'");
	while (res->next()) {
		SQLString checkUsername = res->getString("user_name");
...
QStringList getInfo(QString user) {
	QStringList info;
	CONNECTDB
		con->setSchema(DBACC);
	...
	QString nickname = res->getString("nickname").c_str();
	QString student_ID = res->getString("student_ID").c_str();
	QString email = res->getString("email").c_str();
	delete pstmt;
	delete res;
	DISCONNECTDB
		info << phone << nickname << student_ID << email;
	return info;
}

void writeDB(QString user_now, QString pass_now, QString phone_now, QString nickname_now, QString student_ID_now, QString email_now) {
	CONNECTDB
		con->setSchema(DBACC);
	...
	pstmt->setString(4, nickname_now.toLocal8Bit().constData());
	pstmt->setString(5, student_ID_now.toLocal8Bit().constData());
	pstmt->setString(6, email_now.toLocal8Bit().constData());
	pstmt->execute();
	delete pstmt;
	DISCONNECTDB
}
...
void getBook(QListWidgetItem* a) {
	...
	delete pstmt;
	pstmt = con->prepareStatement("call book_admin.write_request(?,?,?,?,?)");
	pstmt->setString(1,QUuid::createUuid().toString().toLocal8Bit().constData());
	pstmt->setString(2, book_name.toLocal8Bit().constData());
	pstmt->setString(3, author.toLocal8Bit().constData());
...
void delBook(QString name) {
	CONNECTDB
		con->setSchema(DBBOOK);
	pstmt = con->prepareStatement("delete from book where binary book_name = (?)");
	pstmt->setString(1, name.toLocal8Bit().constData());
	pstmt->execute();
	delete pstmt;
	DISCONNECTDB
...
QString getbookID(QString book) {
	CONNECTDB
		con->setSchema(DBBOOK);
	pstmt = con->prepareStatement("select id from book where binary book_name = (?)");
	pstmt->setString(1, book.toLocal8Bit().constData());
	res = pstmt->executeQuery();
	res->next();
	QString value = res->getString("id").c_str();
	delete pstmt;
	delete res;
	DISCONNECTDB
		return value;
...
bool checkAccAvailable(QString a) {
	CONNECTDB
		con->setSchema(DBACC);
	pstmt = con->prepareStatement("select available from account where binary user_name = (?)");
	pstmt->setString(1, a.toLocal8Bit().constData());
	res = pstmt->executeQuery();
	res->next();
	return compareString(res->getString("available").c_str(), "1");
}

int getBorrowNum(QString user) {
	int value = 0;
	CONNECTDB
		con->setSchema(DBBOOK);
	pstmt = con->prepareStatement("select * from borrowbook where binary owner = (?)");
	pstmt->setString(1,user.toLocal8Bit().constData());
	res = pstmt->executeQuery();
	while (res->next())
		value++;
...
int getRequestNum(QString user) {
	int value = 0;
	CONNECTDB
		con->setSchema(DBBOOK);
	pstmt = con->prepareStatement("select * from request where binary account = (?) and available = '1'");
	pstmt->setString(1, user.toLocal8Bit().constData());
	res = pstmt->executeQuery();
	while (res->next())
		value++;
...
bool checkRequestExist(QString a, QString user) {
	bool value = 0;
	CONNECTDB
		con->setSchema(DBBOOK);
	pstmt = con->prepareStatement("select book_name from request where binary account = (?) and available = '1'");
	pstmt->setString(1, user.toLocal8Bit().constData());
	res = pstmt->executeQuery();
	while (res->next()) {
		if (compareString(res->getString("book_name").c_str(), a)) {
...
QStringList getBookInfo(QString book) {
	QStringList value;
	CONNECTDB
		con->setSchema(DBBOOK);
	pstmt = con->prepareStatement("select * from book where binary book_name = (?)");
	pstmt->setString(1, book.toLocal8Bit().constData());
	res = pstmt->executeQuery();
	res->next();
	QString id = res->getString("id").c_str();
	QString author = res->getString("author_name").c_str();
	QString type = res->getString("type").c_str();
	QString release_date = res->getString("release_date").c_str();
	QString rating = res->getString("rating").c_str();
	QString link = res->getString("link").c_str();
	QString summary = res->getString("summary").c_str();
	QString review = res->getString("review").c_str();
	delete pstmt;
	delete res;
	DISCONNECTDB
		value << id << book << author << type << release_date << rating << link << summary << review;
		return value;
}